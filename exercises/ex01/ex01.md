...
12. What happens if you turn on optimization using the flag `-O2`?
  - The output becomes a lot more terse; several steps are "skipped," so to speak--presumably when the compiler thinks that they weren't necessary.
...
without optimization.  What effect does it have when you print `x`?
  - The optimization causes it to "skip" the variable assignment, near as I can tell.
...
optimization.  What happens to `x` and `y`? What conclusions can you draw about how optimization works?

  - Without optimization, a new address is chosen to hold a "y" value, in addition to the "x" value. With optimization, "y" is set outright--since it's the only value that needs to be referred to later in the program. From this, I assume that optimization looks for opportunities to reduce program overhead by "condensing" any frontloadable computation.
